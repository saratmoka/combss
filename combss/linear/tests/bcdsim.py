...
def bulk_sim_given_dataV0(n, p, q, beta_type, K0, snr, corr,
	...
	df.to_csv("./bcdtests/COMBSSV0-case-%s-n-%d-p-%s-q-%s-corr-%s-ninit-%s-snr-%s-ndatasets-%s-nlam-%s-eta-%s.csv" %(beta_type, n, p, q, corr, n_tinit, snr,  n_datasets, nlam, eta))
	return df
...
def bulk_sim_given_dataV1(n, p, q, beta_type, K0, snr, corr,
	...
	df = pd.DataFrame(bulk_results, columns = names)
	df.to_csv("./bcdtests/COMBSSV1-case-%s-n-%d-p-%s-q-%s-corr-%s-ninit-%s-snr-%s-ndatasets-%s-nlam-%s-eta-%s.csv" %(beta_type, n, p, q, corr, n_tinit, snr,  n_datasets, nlam, eta))
	return df
...
save the corresponding results in a file
"""
def bulk_sim_given_dataV2(n, p, q, beta_type, K0, snr, corr,
					 t_init = [],
					 delta_frac = 1,
					 n_datasets=1, 
					 nlam=50,
	...
	df = pd.DataFrame(bulk_results, columns = names)
	df.to_csv("./bcdtests/COMBSSV2-case-%s-n-%d-p-%s-q-%s-corr-%s-ninit-%s-snr-%s-ndatasets-%s-nlam-%s-eta-%s.csv" %(beta_type, n, p, q, corr, n_tinit, snr,  n_datasets, nlam, eta))
	return df
...
snr = 4
print('SNR = ', snr)
dfV0 = bulk_sim_given_dataV0(n, p, q, beta_type, K0, snr, corr,
                     t_init = t_init,
...
snr = 4
print('SNR = ', snr)
dfV2 = bulk_sim_given_dataV2(n, p, q, beta_type, K0, snr, corr,
                     t_init = t_init,
                     delta_frac=delta_frac,
                     n_datasets=n_datasets, 
                     nlam=nlam,
