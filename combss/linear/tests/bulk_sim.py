...
def bulk_simV1(n, p, q, beta_type, K0, snr, corr,
	...
	df = pd.DataFrame(bulk_results, columns = names)
	df.to_csv("./RESULTS/COMBSSV1-case-%s-n-%d-p-%s-q-%s-corr-%s-ninit-%s-snr-%s-ndatasets-%s-nlam-%s-eta-%s.csv" %(beta_type, n, p, q, corr, n_tinit, snr,  n_datasets, nlam, eta))
	return
# %%
...
def bulk_simV2(n, p, q, beta_type, K0, snr, corr,
	...
	df = pd.DataFrame(bulk_results, columns = names)
	df.to_csv("./RESULTS/COMBSSV2-case-%s-n-%d-p-%s-q-%s-corr-%s-ninit-%s-snr-%s-ndatasets-%s-nlam-%s-eta-%s.csv" %(beta_type, n, p, q, corr, n_tinit, snr,  n_datasets, nlam, eta))
	return
