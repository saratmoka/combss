...
def bulk_simV2(n, p, q, beta_type, K0, snr, corr,
	...
	file_path = "./BCD-Block/COMBSSV2-BCD_a-case-%s-n-%d-p-%s-q-%s-corr-%s-ninit-1-snr-%s-ndatasets-%s-nlam-%s-eta-%s.csv" %(beta_type, n, p, q, corr, snr,  n_datasets, nlam, eta)
...
def bulk_simV3(n, p, q, beta_type, K0, snr, corr,
					 t_init = [],
					 delta_frac = 1,
					 n_datasets=1, 
					 nlam=25,
	...
	file_path = "./var3_test/COMBSSV0-Coordinate-100-case-%s-n-%d-p-%s-q-%s-corr-%s-ninit-1-snr-%s-ndatasets-%s-nlam-%s-eta-%s.csv" %(beta_type, n, p, q, corr, snr,  n_datasets, nlam, eta)
	...
	for j in tqdm(range(n_datasets)):
		...
		result1 = variant3.combssV3(X_train, y_train, X_test, y_test, t_init=t_init[0], 
						delta_frac=delta_frac,  q = q, nlam = nlam, eta=eta, cg_maxiter = None)
		"""
		Note that,
			result1 = [model_opt, mse_opt, beta_opt, lam_opt, time]
		...
		for i in range(n_tinit-1):
			...
			result_temp = variant3.combssV3(X_train, y_train, X_test, y_test, t_init=t_init[i+1], 
								 delta_frac=delta_frac, q = q, nlam = nlam, eta=eta, cg_maxiter = None)
...
def bulk_simV4(n, p, q, beta_type, K0, snr, corr,
					 t_init = [],
					 delta_frac = 1,
					 n_datasets=1, 
					 nlam=25,
	...
	for j in tqdm(range(n_datasets)):
		...
		result1 = variant4.combssV4(X_train, y_train, X_test, y_test, t_init=t_init[0], 
						delta_frac=delta_frac,  q = q, nlam = nlam, eta=eta, cg_maxiter = None)
		"""
		Note that,
			result1 = [model_opt, mse_opt, beta_opt, lam_opt, time]
		...
		for i in range(n_tinit-1):
			...
			result_temp = variant4.combssV4(X_train, y_train, X_test, y_test, t_init=t_init[i+1], 
								 delta_frac=delta_frac, q = q, nlam = nlam, eta=eta, cg_maxiter = None)
...
def bulk_simV5(n, p, q, beta_type, K0, snr, corr,
					 t_init = [],
					 delta_frac = 1,
					 n_datasets=1, 
					 nlam=25,
	...
	for j in tqdm(range(n_datasets)):
		...
		result1 = variant5.combssV5(X_train, y_train, X_test, y_test, t_init=t_init[0], 
						delta_frac=delta_frac,  q = q, nlam = nlam, eta=eta, cg_maxiter = None)
		"""
		Note that,
			result1 = [model_opt, mse_opt, beta_opt, lam_opt, time]
		...
		for i in range(n_tinit-1):
			...
			result_temp = variant5.combssV5(X_train, y_train, X_test, y_test, t_init=t_init[i+1], 
								 delta_frac=delta_frac, q = q, nlam = nlam, eta=eta, cg_maxiter = None)
...
def bulk_simV6(n, p, q, beta_type, K0, snr, corr,
					 t_init = [],
					 delta_frac = 1,
					 n_datasets=1, 
					 nlam=25,
	...
	file_path = "./splitting_test/COMBSSV6-case-%s-n-%d-p-%s-q-%s-corr-%s-ninit-1-snr-%s-ndatasets-%s-nlam-%s-eta-%s.csv" %(beta_type, n, p, q, corr, snr,  n_datasets, nlam, eta)
