...
def bulk_simV0(n, p, q, beta_type, K0, snr, corr,
	...
	df.to_csv("./finalResults/COMBSSV0-case-%s-n-%d-p-%s-q-%s-corr-%s-ninit-%s-snr-%s-ndatasets-%s-nlam-%s-eta-%s.csv" %(beta_type, n, p, q, corr, n_tinit, snr,  n_datasets, nlam, eta))
	return
...
def bulk_sim_normalised(n, p, q, beta_type, K0, snr, corr,
	...
	df = pd.DataFrame(bulk_results, columns = names)
	df.to_csv("./finalResults/COMBSSV0-normalised-case-%s-n-%d-p-%s-q-%s-corr-%s-ninit-%s-snr-%s-ndatasets-%s-nlam-%s-eta-%s.csv" %(beta_type, n, p, q, corr, n_tinit, snr,  n_datasets, nlam, eta))
	return
...
def bulk_sim_improved(n, p, q, beta_type, K0, snr, corr,
	...
	df = pd.DataFrame(bulk_results, columns = names)
	df.to_csv("./finalResults/COMBSSImproved-case-%s-n-%d-p-%s-q-%s-corr-%s-ninit-%s-snr-%s-ndatasets-%s-nlam-%s-eta-%s.csv" %(beta_type, n, p, q, corr, n_tinit, snr,  n_datasets, nlam, eta))
	return
...
def bulk_sim_normalised_improved(n, p, q, beta_type, K0, snr, corr,
	...
	df = pd.DataFrame(bulk_results, columns = names)
	df.to_csv("./finalResults/COMBSSV0-Normalised-Improved-case-%s-n-%d-p-%s-q-%s-corr-%s-ninit-%s-snr-%s-ndatasets-%s-nlam-%s-eta-%s.csv" %(beta_type, n, p, q, corr, n_tinit, snr,  n_datasets, nlam, eta))
	return
...
def bulk_simV2(n, p, q, beta_type, K0, snr, corr,
	...
	df = pd.DataFrame(bulk_results, columns = names)
	df.to_csv("./v2comp/sc-5-50/COMBSSV2-case-%s-n-%d-p-%s-q-%s-corr-%s-ninit-%s-snr-%s-ndatasets-%s-nlam-%s-eta-%s-deltazero.csv" %(beta_type, n, p, q, corr, n_tinit, snr,  n_datasets, nlam, eta))
	return
